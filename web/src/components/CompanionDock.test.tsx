...
describe("CompanionDock", () => {
  ...
  it("supports jump, collapse, empty/error states, and clear recent", () => {
    ...
    expect(screen.getByRole("button", { name: "Daddy陪读" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Daddy陪读" }));
    ...
    expect(screen.getByText("Daddy还没留下短评。")).toBeInTheDocument();
  ...
  it("lets the user manually save a pending companion draft", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "保存Daddy短评" }));
