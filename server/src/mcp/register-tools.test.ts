...
describe("tool descriptors", () => {
  it("binds the current UI resource to the v50 and compatibility render tools", () => {
    expect(READING_NEST_URI).toBe("ui://ss-reading-nest/app-v50.html");
    expect(READING_NEST_TOOL_NAME).toBe("open_reading_nest_v50");
    expect(TOOL_CONFIGS.open_reading_nest_v50._meta?.ui).toEqual({
      resourceUri: READING_NEST_URI
    });
    expect(TOOL_CONFIGS.open_reading_nest_v50._meta?.["ui/resourceUri"]).toBe(
      READING_NEST_URI
    );
    expect(TOOL_CONFIGS.open_reading_nest_v50._meta?.["openai/outputTemplate"]).toBe(
      READING_NEST_URI
    );
    ...
    for (const [name, config] of Object.entries(TOOL_CONFIGS)) {
      if (
        ...
        name !== "open_reading_nest_v49" &&
        name !== "open_reading_nest_v48" &&
        name !== "open_reading_nest_v47" &&
        name !== "open_reading_nest_v46" &&
        ...
        name !== "complete_pending_companion_work_v46" &&
  ...
  it("returns the component-only source endpoint for the rendered widget", async () => {
    ...
    const result = (await handlers.get("open_reading_nest_v50")?.()) as {
    ...
    expect(handlers.has("open_reading_nest_v48")).toBe(true);
  ...
  it("opens the earliest durable backlog instead of skipping from paragraph 72 to 74", async () => {
    ...
    registerReadingTools(server as never, service as never, cloudSource as never);
    const result = await handlers.get("open_reading_nest_v50")?.();
  ...
  it("exposes book management and threaded annotation tools", () => {
    expect(Object.keys(TOOL_CONFIGS)).toHaveLength(68);
  ...
  it("saves and reloads annotations through stable legacy-catalog tools", async () => {
    ...
    let annotationAutoplayAttempts = 0;
    const companionAutoplayService = {
      completeAnnotation: async () => {
        annotationAutoplayAttempts += 1;
        return { completed: false, kind: "annotation", reason: "generation_failed" } as const;
      }
    };
