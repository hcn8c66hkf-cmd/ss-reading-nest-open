...
describe("tool descriptors", () => {
  it("binds the current UI resource to the v46 and compatibility render tools", () => {
    expect(READING_NEST_URI).toBe("ui://ss-reading-nest/app-v46.html");
    expect(READING_NEST_TOOL_NAME).toBe("open_reading_nest_v46");
    expect(TOOL_CONFIGS.open_reading_nest_v46._meta?.ui).toEqual({
      resourceUri: READING_NEST_URI
    });
    expect(TOOL_CONFIGS.open_reading_nest_v46._meta?.["ui/resourceUri"]).toBe(
      READING_NEST_URI
    );
    expect(TOOL_CONFIGS.open_reading_nest_v46._meta?.["openai/outputTemplate"]).toBe(
      READING_NEST_URI
    );
  ...
  it("returns the component-only source endpoint for the rendered widget", async () => {
    ...
    const result = (await handlers.get("open_reading_nest_v46")?.()) as {
  ...
  it("opens the earliest durable backlog instead of skipping from paragraph 72 to 74", async () => {
    ...
    registerReadingTools(server as never, service as never, cloudSource as never);
    const result = await handlers.get("open_reading_nest_v46")?.();
  ...
  it("exposes book management and threaded annotation tools", () => {
    expect(Object.keys(TOOL_CONFIGS)).toHaveLength(64);
